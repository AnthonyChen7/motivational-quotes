...
//   test.only('should display initial chat UI elements', async ({ page }) => {
...
//   test('should send message when clicking a preset prompt button', async ({ page }) => {
//     // Mock the streaming API response format
//     await page.route('**/api/chat', async route => {
//       const streamResponse = createMockChatResponse(
//         'Rest is not giving up, it is preparing for the journey ahead.'
//       );
...
//     // Click on the first preset prompt
//     await page.getByRole('button', { name: "I'm feeling tired" }).click();
...
//     await expect(page.getByText("I'm feeling tired")).toBeVisible();
//   });
